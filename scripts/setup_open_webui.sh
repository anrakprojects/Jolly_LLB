...
# Bootstrap Open WebUI against Jolly LLB's OpenAI-compatible API server.
#
...
#   HERMES_API_MODEL_NAME='Jolly LLB'

OPEN_WEBUI_PORT="${OPEN_WEBUI_PORT:-8080}"
OPEN_WEBUI_HOST="${OPEN_WEBUI_HOST:-127.0.0.1}"
OPEN_WEBUI_NAME="${OPEN_WEBUI_NAME:-Jolly LLB WebUI}"
...
HERMES_API_MODEL_NAME="${HERMES_API_MODEL_NAME:-Jolly LLB}"
...
install_systemd_user_service() {
  ...
  cat > "$unit" <<EOF
[Unit]
Description=Open WebUI connected to Jolly LLB
