# Jolly LLB Security Policy

This document describes Jolly LLB's trust model, names the one
...
security vulnerabilities. **Jolly LLB does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Jolly LLB access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Jolly LLB's documentation
...
Jolly LLB supports two OS-level isolation postures. They address
...
Jolly LLB supports this in two ways:

- **Jolly LLB's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Jolly LLB's in-process heuristics
...
Jolly LLB filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Jolly LLB
itself. Bugs in Jolly LLB's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Jolly LLB does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Jolly LLB's own documentation, or reasonable
  operator expectations would predict — including cases where
  Jolly LLB has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Jolly LLB's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Jolly LLB. Bugs in Hermes
...
- Jolly LLB includes supply-chain guards for MCP server
