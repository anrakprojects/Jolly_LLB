...
function downloadInstallScript(commit, destPath) {
  ...
  // The fork (anrakprojects/Jolly_LLB) is PRIVATE — its commit SHA 404s on
  // raw.githubusercontent for any end user without a token, so the stamped
  // `commit` can't be used to fetch the installer. Pin to a public NousResearch
  // ref so the runtime bootstraps on any machine. (Boots the upstream Nous
  // runtime by design — see the AnrakLegal desktop rebrand notes.)
  const PUBLIC_INSTALL_REF = 'v2026.6.5'
  void commit
  const url = `https://raw.githubusercontent.com/NousResearch/hermes-agent/${PUBLIC_INSTALL_REF}/scripts/${scriptName}`
